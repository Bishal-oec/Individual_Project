...
import io.skyway.Peer.MeshRoom;
...
public class FlutterSkywayPeer {
	...
	private MediaStream _captureStream;
...
//	private Room _room;
	...
	public void join(@NonNull final String roomName, final RoomOption.RoomModeEnum mode)
		...
		if ((_room == null) || !roomName.equals(_roomName) || !mode.equals(_roomMode)) {
			...
			// Join Mesh Room for replacement stream
			_room = (MeshRoom) _peer.joinRoom(roomName, option);
	 ...
	 * @throws IllegalArgumentException 描画先のCanvasが見つからなかった
	 */
	public void resumeCaptureStream(final int localVideoId)
		throws IllegalArgumentException {

		if (DEBUG) Log.v(TAG, "resumeCaptureStream:" + localVideoId);
		if (_localStream != null && _localCanvas != null) {
			_localStream.removeVideoRenderer(_localCanvas, 0);
		}
		final MediaConstraints constraints = new MediaConstraints();
		...
		_localStream = Navigator.getUserMedia(constraints);
		_room.replaceStream(_localStream);
		_localCanvas = getCanvas(localVideoId);
		if (_localCanvas != null) {
			_localVideoId = localVideoId;
			_localStream.addVideoRenderer(_localCanvas, 0);
	 ...
	 * @throws IllegalArgumentException 描画先のCanvasが見つからなかった
	 */
	public boolean sendData(final int localVideoId, String data)
			throws IllegalArgumentException {
		boolean isSendData = false;

		if (DEBUG) Log.v(TAG, "sendData:" + data );
		if (_room != null) {
			isSendData = _room.send(data);
		} else {
			// ここにくるのはプログラムミス
			throw new IllegalArgumentException();
		}
	...
	/**
	 * 画面共有を完了してロカルvideoに戻る
	 */
	public void stopCaptureStream(final int localVideoId)
			throws IllegalArgumentException {

		if (DEBUG) Log.v(TAG, "stopCaptureStream:" + localVideoId);
		if (_localStream != null && _localCanvas != null) {
			_localStream.removeVideoRenderer(_localCanvas, 0);
		}
		_localStream.close();
	 ...
	 * キャメラ切り替え
	 */
	public boolean switchCameraStream()
			throws IllegalArgumentException {
	...
	/**
	 * 画面共有を開始してロカルvideoに画面キャプチャーに変わる
	 */
	public void startCaptureStream(final int localVideoId)
			throws IllegalArgumentException {

		if (DEBUG) Log.v(TAG, "startCaptureStream:" + localVideoId);
		if (_localStream != null && _localCanvas != null) {
			_localStream.removeVideoRenderer(_localCanvas, 0);
		}
			final MediaConstraints constraints = new MediaConstraints();
			...
			_captureStream = Navigator.getDisplayMedia(constraints, mediaProjectionPermissionResultData  ,new MediaProjectionCallback());
			_room.replaceStream(_captureStream);
			_localStream = _captureStream;
			_localCanvas = getCanvas(localVideoId);
			if (_localCanvas != null) {
				_localVideoId = localVideoId;
				_localStream.addVideoRenderer(_localCanvas, 0);
	 ...
	 * キャプチャ許可データを取得する
	 */
	public static void handleActivityResult(Intent data) {
		if(data != null){
...
//--------------------------------------------------------------------------------
	private void setRoomCallback(@NonNull final MeshRoom room) {
		...
		room.on(Room.RoomEventEnum.DATA, new OnCallback() {
            ...
            public void onCallback(Object object) {
                ...
                if (msg.data instanceof String)	{
                    String messageData = (String)msg.data;
					Log.i("Data", messageData);

					final Map<String, String> message
						= createMessage(Const.SkywayEvent.OnData);
