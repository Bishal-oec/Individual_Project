...
import 'package:online_communication_app/call_screen.dart';
...
class _SelectorScreenState extends State<SelectorScreen> {
  ...
  // static  String baseUrl = "http://192.168.5.152/TelemedicineSystemAPI"; // ロカル環 URL
                    ...
                    children: <Widget>[
                      ...
                      Expanded(
                        child: ElevatedButton(
                          ...
                          child: _isLoading // Show CircularProgressIndicator if isLoading is true
  ...
  Future<User?> loginUser(String userId, String password) async {
   try {
    ...
    if (response.statusCode == 200) {
      ...
      // throw an exception.
      final errorMessage = jsonDecode(response.body)['error']['message'] ?? 'An error occurred';
      setState(() {
    ...
    showDialog(
      context: context, // You need to have access to the BuildContext for this
      builder: (context) {
        return AlertDialog(
  ...
  Future<void> loadSystemName(String loginToken) async {
    ...
    var response = await http.get(Uri.parse("$baseUrl/User/SystemName/"), headers: headers);
    if (response.statusCode == 200) {
  ...
  void _setTokenKengenKbn(final String token, final String loginKengenKbn, final int userNoId) async {
      final SharedPreferences prefs = await SharedPreferences.getInstance();
